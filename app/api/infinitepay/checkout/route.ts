...
export async function POST(req: Request) {
  try {
    ...
    console.log("Gerando link InfinitePay:", {
      saleId,
      amount,
      customer,
      address,
      ...
      items:
        items && items.length > 0
          ...
          : [
              {
                quantity: 1,
                price: Math.round(amount * 100), // Convert BRL to cents
    ...
    if (
      ...
      if (customer.phone_number)
        payload.customer.phone_number = customer.phone_number; // Already formatted
    }

    // Adiciona address se fornecido
    if (address && address.cep && address.number) {
      payload.address = {
        cep: address.cep, // Numbers only
        number: address.number,
      };

      // Optional InfinitePay API fields
      if (address.street) payload.address.street = address.street;
      if (address.neighborhood)
        payload.address.neighborhood = address.neighborhood;
    ...
    return NextResponse.json(
      { url: data.url, order_nsu: payload.order_nsu },
      { status: 200 },
    );
  } catch (e) {
