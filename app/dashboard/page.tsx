...
export default function DashboardPage() {
    ...
    try {
      ...
      if (result.success) {
        const futureEvents = (result.events as CalendarEvent[])
          .filter(
            (event: CalendarEvent) => new Date(event.start.dateTime) >= now,
          )
          .sort(
        ...
        setEventsError(
          result.error || "Erro ao carregar próximos agendamentos",
        );
      }

      // Fetch events from the last 30 days to calculate popular services
      const thirtyDaysAgo = new Date();
      thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
      const pastResult = await listCalendarEvents(
  ...
  // Actual monthly revenue (last 6 months) based on paid payments
  const now = new Date();
  ...
  const growthPercent =
    (revenueData[revenueData.length - 2]?.revenue ?? 0) > 0
  ...
  function extractClientName(description?: string): string {
    ...
    const clientLine = description
      .split("\n")
      .find((line) => line.startsWith("Cliente:"));
    return (
      clientLine?.replace("Cliente:", "").trim() || "Cliente não identificado"
    );
  }

  function extractPhone(description?: string): string | null {
    if (!description) return null;
    const phoneLine = description
      .split("\n")
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="text-sm font-semibold text-green-600">
                        R${" "}
                        {client.spent.toLocaleString("pt-BR", {
                          minimumFractionDigits: 2,
                        })}
                      </p>
                      <p className="text-xs text-gray-500 mt-0.5">
